...
//#region node_modules/.nitro/vite/services/ssr/assets/profile.server-CBSyRa1R.js
var PROFILE_COLUMNS = "id, full_name, class_level, total_xp, credits, referral_code, phone, guardian_phone, school_name, city, state, board, gender, date_of_birth, preferred_language, goal, onboarding_completed, created_at";
async function getProfileFor(userId) {
	const { data } = await supabaseAdmin.from("profiles").select(PROFILE_COLUMNS).eq("id", userId).maybeSingle();
	return data;
}
...
async function saveProfileDetailsFor(userId, input) {
...
async function getUserDetailFor(userId) {
	const [profile, auth, roles, streak, progress, attempts, unlocks, credits, referrals, badges] = await Promise.all([
		...
		supabaseAdmin.from("lesson_progress").select("lesson_id, completed_at, lessons(title, chapter_id)").eq("user_id", userId).order("completed_at", { ascending: false }).limit(25),
		supabaseAdmin.from("test_attempts").select("id, test_id, score, total, created_at, tests(title)").eq("user_id", userId).order("created_at", { ascending: false }).limit(25),
		supabaseAdmin.from("lesson_unlocks").select("lesson_id, cost, created_at").eq("user_id", userId),
		supabaseAdmin.from("credit_events").select("delta, reason, created_at").eq("user_id", userId).order("created_at", { ascending: false }).limit(25),
...
//#endregion
export { getProfileFor, getUserDetailFor, saveProfileDetailsFor };
